...
**Goal**: Universal CLI wrapper that configures environment variables and runs build 
commands for all supported build systems (Go, Bazel, npm, Maven, Cargo, pip).

**Estimated effort**: 1-2 days

**Philosophy**: This tool is a convenience wrapper. It doesn't replace the actual build
tools — it just sets the correct environment variables to point them at MonoFS. Users
can still run builds manually by setting env vars themselves.

### Design

The CLI has two modes:

1. **Auto-detect mode**: `monofs-build [options]` — detect build system from current dir
2. **Explicit mode**: `monofs-build <type> [options] -- <build command>` — specify build system

### Environment Variables Set Per Build System

| Build System | Primary Env Var | Additional Vars | Virtual Path |
|--------------|----------------|-----------------|--------------|
| **Go** | `GOMODCACHE` | `GONOSUMDB=*`, `GONOSUMCHECK=*`, `GOFLAGS=-mod=mod` | `go-modules/pkg/mod` |
| **Bazel** | `BAZEL_REPOSITORY_CACHE` | - | `bazel-repos` |
| **npm** | `NPM_CONFIG_CACHE` | `NPM_CONFIG_PREFER_OFFLINE=true` | `npm-cache` |
| **Maven** | `MAVEN_REPO` | - | `maven-repo` |
| **Cargo** | `CARGO_HOME` | `CARGO_NET_OFFLINE=true` | `cargo-home` |
| **pip** | `PIP_CACHE_DIR` | `PIP_NO_INDEX=true` | `pip-cache` |

### Features

- **Verification**: Check if MonoFS mount point exists and contains expected paths
- **Passthrough**: All arguments after `--` are passed to the underlying build tool
- **Environment inheritance**: Merges MonoFS vars with existing environment
- **Error messages**: Helpful diagnostics if mount/cache dirs missing
- **Dry-run mode**: `--dry-run` shows what would be executed without running it
...
Find the `BIN_DIR` targets section and add:
...
Also add `monofs-build` to the main `build` target dependencies.
...
# Show help
./bin/monofs-build --help

# Dry run examples
./bin/monofs-build --dry-run go -- build ./...
./bin/monofs-build --dry-run npm -- install
./bin/monofs-build --dry-run --mount=/tmp/monofs cargo -- build

# Auto-detect test (from a Go project directory)
cd /tmp
mkdir -p test-project
cd test-project
echo "module test" > go.mod
/path/to/monofs-build --dry-run build ./...
# Expected: Auto-detected build system: go

# Test with actual MonoFS mount (requires cluster running)
# 1. Start MonoFS
monofs-client --mount=/tmp/monofs --router=localhost:9090 &
...
func main() { println(uuid.New().String()) }
EOF

# 3. Ingest deps
monofs-admin ingest-deps --router=localhost:9090 --file=go.mod --type=go

# 4. Build with monofs-build
monofs-build --mount=/tmp/monofs build ./...
# Expected: successful build using MonoFS GOMODCACHE
...
# Cleanup
fusermount -u /tmp/monofs
```

### Advanced Usage Examples

```bash
# Go with custom flags
monofs-build go -- build -v -race -o myapp ./cmd/myapp

# npm with specific script
monofs-build npm -- run build:prod

# Maven with profiles
monofs-build maven -- clean install -P production

# Cargo with features
monofs-build cargo -- build --release --features=extra

# Bazel with specific targets
monofs-build bazel -- build //src/...

# pip install requirements
monofs-build pip -- install -r requirements.txt

# Show environment for debugging
monofs-build --dry-run --mount=/custom/mount cargo -- build
```
...
jobs:
  build:
    steps:
      - name: Mount MonoFS
        run: |
          monofs-client --mount=/mnt/monofs --router=${{ secrets.MONOFS_ROUTER }} &
          sleep 5
      
      - name: Ingest dependencies
        run: monofs-admin ingest-deps --file=go.mod --type=go --router=${{ secrets.MONOFS_ROUTER }}
      
      - name: Build with MonoFS
        run: monofs-build --mount=/mnt/monofs build -v ./...
      
      - name: Test
        run: monofs-build --mount=/mnt/monofs test ./...
```
