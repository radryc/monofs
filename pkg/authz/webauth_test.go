...
func TestWebAuthFullFlow(t *testing.T) {
	...
	wa.LogoutHandler(logoutRec, logoutReq)
	cleared := false
	for _, c := range logoutRec.Result().Cookies() {
		if c.Name == defaultSessionCK && c.MaxAge < 0 {
			cleared = true
		}
	}
	if !cleared {
...
func TestWebAuthUsesPersistentStoreFromEnv(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("MONOFS_SESSION_DIR", dir)
	...
	wa, err := NewWebAuthenticator(context.Background(), WebAuthConfig{
		...
		Verifier: tokenVerifierFunc(func(_ context.Context, _ string) (Identity, error) {
			return Identity{}, nil
		}),
	})
	if err != nil {
	...
	if _, ok := wa.cfg.Sessions.(*PersistentSessionStore); !ok {
		t.Fatalf("expected PersistentSessionStore when MONOFS_SESSION_DIR is set, got %T", wa.cfg.Sessions)
	}

	if _, statErr := os.Stat(dir); statErr != nil {
		t.Fatalf("expected session directory to exist, stat error: %v", statErr)
	}
}

func TestWebAuthFallsBackToMemoryStoreWhenSessionDirInvalid(t *testing.T) {
	t.Setenv("MONOFS_SESSION_DIR", "/dev/null/monofs-authz-sessions")
	...
	if _, ok := wa.cfg.Sessions.(*SessionStore); !ok {
		t.Fatalf("expected in-memory SessionStore fallback, got %T", wa.cfg.Sessions)
	}
}
