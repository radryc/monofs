...
func NewWebAuthenticator(ctx context.Context, cfg WebAuthConfig) (*WebAuthenticator, error) {
	...
	if cfg.Sessions == nil {
		sessionDir := strings.TrimSpace(os.Getenv("MONOFS_SESSION_DIR"))
		if sessionDir != "" {
			ps, err := NewPersistentSessionStore(filepath.Clean(sessionDir), 12*time.Hour)
			if err == nil {
				cfg.Sessions = ps
			} else {
...
func (w *WebAuthenticator) CallbackHandler(rw http.ResponseWriter, r *http.Request) {
	...
	// Store the id_token directly — any router can verify it without shared session state.
...
func (w *WebAuthenticator) identityFromRequest(r *http.Request) Identity {
	if c, err := r.Cookie(w.cfg.CookieName); err == nil && c.Value != "" {
		// JWT cookie: verifiable by any router instance without shared state.
		if id, err := w.cfg.Verifier.Verify(r.Context(), c.Value); err == nil {
			return id
		}
		// Fallback: opaque session ID (break-glass tokens / pre-existing sessions).
