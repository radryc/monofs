...
- Terminal-backed commands for the sibling `../scripts` operational flow and local MonoFS binaries.
...
- The sibling `../scripts` repo, or a custom `monofs.scriptsRepoPath` setting.
...
- `monofs.scriptsRepoPath`: Absolute path to the sibling deployment scripts repo.
...
- `scripts/image build --partition dev-workspace` packages the extension into the `dev-workspace-vscode` image.
