...
func (r *Router) buildRoutersData() *RoutersData {
	...
	for _, peer := range peers {
		...
		go func() {
			...
			status, statusErr := fetchRouterStatus(client, normalizedURL, peerToken)
			repos, reposErr := fetchRouterRepositories(client, normalizedURL, peerToken)
...
func fetchRouterStatus(client *http.Client, baseURL, bearerToken string) (*StatusData, error) {
...
func fetchRouterRepositories(client *http.Client, baseURL, bearerToken string) (*RepositoriesData, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Second)
	defer cancel()
