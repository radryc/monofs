...
func (r *Router) SetRegistryAuthToken(token string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.registryAuthToken = strings.TrimSpace(token)
	if r.registryAuthToken != "" {
		r.logger.Info("registry api machine token configured")
	}
}

func (r *Router) SetPeerRouterAuthToken(token string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.peerRouterToken = strings.TrimSpace(token)
	if r.peerRouterToken != "" {
		r.logger.Info("peer router machine token configured")
	}
}
